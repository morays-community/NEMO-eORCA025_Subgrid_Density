MODULE inffld
   ...
   !!----------------------------------------------------------------------
   REAL(wp), PUBLIC, ALLOCATABLE, SAVE, DIMENSION(:,:,:)  :: rho_corr    !: dummy field to store 3D inferences
...
CONTAINS
   ...
   INTEGER FUNCTION inffld_alloc()
      ...
      ierr = 0
      !
      ALLOCATE( rho_corr(jpi,jpj,jpk) , STAT=ierr )
      inffld_alloc = ierr
      !
   END FUNCTION
